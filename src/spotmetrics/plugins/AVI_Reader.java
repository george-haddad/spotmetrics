...
//        private final static int   AVIF_ISINTERLEAVED= 0x00000100;  // ignored by this plugin
        ...
        private  int               type0xdb;                 //video stream chunks must have one of these two types (e.g. '00db' for straem 0)
        private  int               type0xdc;                 //video stream chunks must have one of these two types (e.g. '00db' for straem 0)
        ...
        private  long              indexPositionEnd;         //index seek end
        ...
        private  boolean           verbose = false;     //IJ.debugMode;
         ...
         */
        public void openVideo(File videoFile, boolean useVirtualStack) {
                String path = videoFile.getAbsolutePath();
                String fileName = videoFile.getName();
                String fileDir = path.substring(0, path.indexOf(File.separatorChar)+1);
                ...
                try {
                        // read data
                        ImageStack stack = makeStack(path, firstFrame, lastFrame, useVirtualStack, convertToGray, flipVertical);
                }
                catch (Exception e) {
                        error(exceptionMessage(e));
                        return;
                ...
                if (stack.getSize() == 0) {
                        ...
                        if (firstFrame > 1 || lastFrame != 0) {
                                rangeText = "\nin Range " + firstFrame + (lastFrame > 0 ? " - " + lastFrame : " - end");
                        }
        ...
        public synchronized ImageProcessor getProcessor(int n) {
                ...
                try {
                        ...
                        long[] frameInfo = (frameInfos.get(n - 1));
        ...
        public String getSliceLabel(int n) {
                ...
                return frameLabel((frameInfos.get(n - 1))[2]);
        }
        ...
        private void readAVI(String path) throws Exception, IOException {
                if (!headerOK) {
                        // we have not read the header yet?
                        openAndReadHeader(path);
                }

                // taking previously elapsed time into account
                startTime += System.currentTimeMillis();
                /** MOVED UP HERE BY JSP */
                if (lastFrame > 0) {
                        // last frame number to read: from Dialog
                        lastFrameToRead = lastFrame;
                }

                if (lastFrame < 0 && dwTotalFrames > 0) {
                        // negative means "end frame minus ..."
                        lastFrameToRead = dwTotalFrames + lastFrame;
                }

                if (lastFrameToRead < firstFrame) {
                        // no frames to read
                        return;
                }
                ...
                if (isVirtual || firstFrame > 1) {
                        // avoid scanning frame-by-frame where we only need the positions
                        // holds frame positions, sizes and time since start
                        frameInfos = new Vector<long[]>(100);
                        long nextPosition = -1;
                        if (indexPosition > 0) {
                        ...
                        if (hasIndex && frameInfos.size() == 0) {
                                // got nothing from indx, attempt to read AVI 1 index 'idx1'
                                raFile.seek(headerPositionEnd);

                                // go behind the 'movi' list
                                moviPosition = findFourccAndSkip(FOURCC_movi, true, fileSize);
                                if (moviPosition > 0) {
                ...
                if (isVirtual && frameInfos.size() > 0) {
                        // Virtual Stack only needs reading the index
                        return;
                }

                // Read AVI (movie data) frame by frame - if no index tag is 
                // present the pointers for the virtual AVI stack will be read here
                raFile.seek(headerPositionEnd);

                if (firstFrame > 1 && frameInfos.size() > 0) {
                        long[] frameInfo = frameInfos.get(0);

                        // chunk starts 8 bytes before frame data
                        raFile.seek(frameInfo[0] - 8);
        ...
        private long findFourccAndRead(int fourcc, boolean isList, long endPosition, boolean throwNotFoundException) throws Exception, IOException {
                ...
                do {
                        ...
                        if (verbose) {
                                IJ.log("Searching for '" + fourccString(fourcc) + "', found " + fourccString(type) + "' " + posSizeString(nextPos - size, size));
                        }
        ...
        private long findFourccAndSkip(int fourcc, boolean isList, long endPosition) throws IOException {
                while (true) {
                        ...
                        // note that 'size' of a list includes the 'type' that follows now
                        ...
                        if (verbose) {
                                IJ.log("Searching for (to skip) '" + fourccString(fourcc) + "', found " + fourccString(type) + "' " + posSizeString(chunkPos, size));
                        }
        ...
        private boolean readContents(int fourcc, long endPosition) throws Exception, IOException {
                switch (fourcc) {
                        ...
                        case FOURCC_strl: {
                                ...
                                // try reading indx for counting number of entries
                                indexForCountingOnly = true;
        ...
        void readAviHeader() throws Exception, IOException {
                ...
                // in newer avi formats, this is dwPaddingGranularity?
                dwReserved1 = readInt();
                dwFlags = readInt();
        ...
        void readStreamHeader() throws Exception, IOException {
                ...
                // '01db' for stream 1, etc. (inverse byte order!)
                type0xdb = FOURCC_00db + (streamNumber << 8);

                // '01dc' for stream 1, etc.
                type0xdc = FOURCC_00dc + (streamNumber << 8);
        }
        ...
        private void readAvi2Index(long endPosition) throws Exception, IOException {
                ...
                if (bIndexType == AVI_INDEX_OF_INDEXES) {
                        ...
                        for (int i = 0; i < nEntriesInUse; i++) {
                                ...
                                // ignored; not trustworthy anyhow
                                int dwDuration = readInt();
                                if (verbose) {
                        ...
                        if (dwChunkId != type0xdb && dwChunkId != type0xdc) {
                                // not the stream we search for? (should not happen)
                                if (verbose) {
                                        IJ.log("INDEX ERROR: SKIPPED ix00, wrong stream number or type, should be " + fourccString(type0xdb) + " or " + fourccString(type0xdc));
                                }
        ...
        private void readOldFrameIndex(long endPosition) throws Exception, IOException {
                // difference between absolute frame address and address given in idx1
                int offset = -1;

                // dwOffset may be w.r.t. file start or w.r.t. 'movi' list.
                int[] offsetsToTry = new int[] { 0, (int) moviPosition };
                while (true) {
                        ...
                        if ((dwChunkId == type0xdb || dwChunkId == type0xdc) && dwSize > 0) {
                                if (offset < 0) {
                                        ...
                                        for (int i = 0; i < offsetsToTry.length; i++) {
                                                ...
                                                raFile.seek(pos);
                                                // see whether this offset points to the desired chunk
                                                int chunkIdAtPos = readInt();
        ...
        void readBitMapInfo(long endPosition) throws Exception, IOException {
                ...
                switch (biCompression) {
                        ...
                        case NO_COMPRESSION_RAW: {
                                ...
                                // RGB mode is usually bottom-up, negative height signals top-down
                                dataTopDown = biHeight < 0;

                                // we don't support 1, 2 and 4 byte data
                                allowedBitCount = 8 | BITMASK24 | 32;
                        ...
                        default: {
                                throw new Exception("Unsupported compression: " + Integer.toHexString(biCompression) + (biCompression >= 0x20202020 ? " '" + fourccString(biCompression) + "'" : ""));
                        }
                }

                // convert "24" to a flag
                int bitCountTest = biBitCount == 24 ? BITMASK24 : biBitCount;
                if (allowedBitCount != 0 && (bitCountTest & allowedBitCount) == 0) {
                        throw new Exception("Unsupported: " + biBitCount + " bits/pixel for compression '" + fourccString(biCompression) + "'");
                }

                if (biHeight < 0)
                        // negative height was for top-down data in RGB mode
                        biHeight = -biHeight;
        ...
        void readMovieData(long endPosition) throws Exception, IOException {
                ...
                if (verbose) {
                        IJ.log("Searching for stream " + streamNumber + ": '" + fourccString(type0xdb) + "' or '" + fourccString(type0xdc) + "' chunks");
                }

                if (isVirtual) {
                        if (frameInfos == null) {
                                // we might have it already from reading the first chunk holds frame
                                //positions in file (for non-constant frame sizes, should hold long[]
                                //with pos and size)
        ...
        private Object readFixedLengthFrame(RandomAccessFile rFile, int size) throws Exception, IOException {
                if (size < scanLineSize * biHeight) {
                        // check minimum size (fixed frame length format)
        ...
        void unpackGray(byte[] rawData, int rawOffset, byte[] pixels, int byteOffset, int w) {
                ...
                if (dataCompression == 0) {
                        ...
                        for (int i = 0; i < w; i++) {
                                // Non-standard: no scaling from 16-235 to 0-255 here
                                pixels[j++] = rawData[k];
                                k += step;
                        }
                }
                ...
                switch (dataCompression) {
                        ...
                        case YUY2_COMPRESSION: {
                                for (int i = 0; i < w / 2; i++) {
                                        ...
                                        // converts byte range 0...ff to -128 ... 127
        ...
        private void setFramesPerSecond(ImagePlus imp) {
                if (dwMicroSecPerFrame < 1000 && dwStreamRate > 0) {
                        // if no reasonable frame time, get it from rate
        ...
        private String exceptionMessage(Exception e) {
                ...
                if (e.getClass() == Exception.class) {
                        // for "home-built" exceptions: message only
                        msg = e.getMessage();
                }
                else {
                        msg = e + "\n" + e.getStackTrace()[0] + "\n" + e.getStackTrace()[1];
        ...
        final private static byte[] HUFFMAN_TABLES = new byte[] {
        ...
        class raInputStream extends InputStream {
                ...
                public int read() throws IOException {
                        ...
                        if (fixMJPG) {
                                ...
                                if (bufferPointer >= bufferLength) {
                                        // buffer exhausted, no more attempt to fix it
                                        fixMJPG = false;
                                }
                ...
                private void addHuffmanTables() {
                        ...
                        do {
                                // read segment type
                                int code = readShort(offset);
                                if (code == 0xffc4) {
                                        // Huffman table found, nothing to do
                                        return;
                                }
                                ...
                                // read length of this segment
                                segmentLength = readShort(offset);

                                // and skip the segment contents
